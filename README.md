...
1. In order to cater to load of requests we should create a dedicated ingest micro-service to receive the delivery requests and ingest into the cache ( elastic search, Redis, etc. )
so that it is readily available for click service to identify the delivery ID, as click and install ingestion depends on delivery ID.
2. Once the delivery, click and install are captured using ingest service then we can publish the message ( delivery, click, and install ) on message queue and there will be another
micro-service to actually persist the data in a data store - and sends acknowledgement to the publisher.
3. We should create different topics for delivery, click and install messages.
4. Finally there will be a separate micro-service to cater to statistics request(query-service).

Database -
NOSQL
1) Horizontal scalability possible.
2) https://www.mongodb.com/mongodb-scale - scalability options.
3) https://dzone.com/articles/divide-and-conquer-high-scalability-with-mongodb-t

Load balancing can be used to cater to multiple install requests.. http://horicky.blogspot.com/2010/10/scalable-system-design-patterns.html
